...
class LawExamCrawler:
    ...
    def run(self, max_pages=None, skip_existing=True, batch_size=10, batch_delay=60):
        """執行爬蟲程序"""
        ...
        logger.info(f"共找到 {total_pages} 頁考古題資料")

        # 收集所有考古題資訊
        all_exams = []

        for page_no in tqdm(range(1, total_pages + 1), desc="處理頁面"):
            ...
            if not exams and self.debug:
                logger.warning(f"第 {page_no} 頁沒有找到考古題")
        ...
        # 分批處理
        for i in range(0, len(all_exams), batch_size):
            batch = all_exams[i:i+batch_size]

            logger.info(
                f"處理第 {i//batch_size + 1} 批，共 {len(batch)} 份考古題 (總進度: {i}/{len(all_exams)})")
...
if __name__ == "__main__":
    ...
    if args.fixed_pages:
        ...
        # 修改run方法的調用方式，跳過獲取總頁數
        all_exams = []

        for page_no in tqdm(range(1, max_pages + 1), desc="處理頁面"):
            logger.info(f"正在處理第 {page_no} 頁，共 {max_pages} 頁")
            soup = crawler.get_page(page_no)

            if not soup:
                logger.warning(f"無法獲取第 {page_no} 頁，跳過")
                continue

            exams = crawler.parse_exam_info(soup)
            all_exams.extend(exams)

        logger.info(f"共收集到 {len(all_exams)} 份考古題資訊")

        if len(all_exams) > 0:
            crawler.export_to_csv(all_exams)
            crawler.generate_readme(all_exams)

            # 下載所有PDF文件 - 分批下載以避免429錯誤
            logger.info(
                f"開始下載 {len(all_exams)} 份考古題 (分批下載，每批 {args.batch_size} 份，批次間隔 {args.batch_delay} 秒)...")

            success_count = 0
            fail_count = 0

            # 分批處理
            for i in range(0, len(all_exams), args.batch_size):
                batch = all_exams[i:i+args.batch_size]

                logger.info(
                    f"處理第 {i//args.batch_size + 1} 批，共 {len(batch)} 份考古題 (總進度: {i}/{len(all_exams)})")

                for exam in tqdm(batch, desc=f"批次 {i//args.batch_size + 1}"):
                    if crawler.download_pdf(exam, args.skip_existing):
                        success_count += 1
                    else:
                        fail_count += 1

                # 每批次處理完後等待較長時間，避免觸發429
                if i + args.batch_size < len(all_exams):
                    logger.info(f"批次處理完成，等待 {args.batch_delay} 秒後繼續下一批...")
